...
class StockMoveCancel(models.TransientModel):
    ...
    delete_move = fields.Boolean(
        string='Delete selected stock moves and related quants movements.')
    cancel_move = fields.Boolean(
        string='Cancel selected stock moves and delete related quants movements.')
    ...
    def _action_cancel(self):
        ...
        for move in moves:
            move._do_unreserve()
            siblings_states = (
                        move.move_dest_ids.mapped('move_orig_ids') - move).mapped(
                'state')
            if move.propagate:
                    ...
                    if move.picking_id.sale_id.warehouse_id.delivery_steps == 'pick_ship':
                        if pack_op.location_dest_id.usage == 'customer':
                            outgoing_quant = self.env['stock.quant'].sudo().search(
                                [('product_id', '=', move.product_id.id),
                                 ('location_id', '=', pack_op.location_dest_id.id)])
                            if outgoing_quant:
                                old_qty = outgoing_quant[0].quantity
                                outgoing_quant[
                                    0].quantity = old_qty - move.product_uom_qty
                        else:
                            outgoing_quant = self.env['stock.quant'].sudo().search(
                                [('product_id', '=', move.product_id.id),
                        ...
                        if pack_op.location_dest_id.usage == 'customer':
                            for mvl in move.move_line_ids:
                                if mvl.product_id.tracking != 'none':
                                    outgoing_quant = self.env[
                                        'stock.quant'].sudo().search(
                                        [('product_id', '=', move.product_id.id), (
                                        'location_id', '=',
                                        pack_op.location_dest_id.id),
